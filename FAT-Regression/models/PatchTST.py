...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc):
        ...
        batch_size_nvars, patch_num, d_model = enc_out.shape
        # encoder
        enc_out, _ = self.encoder(enc_out) # enc_out: [(bs * n_vars) x patch_num x d_model]

        enc_out = torch.reshape(enc_out, (bs, n_vars, patch_num, d_model)) # enc_out: [bs x n_vars x patch_num x d_model]
    ...
    def pretrain(self, x_enc, x_mark_enc, batch_x, mask):
        ...
        # return batch_x.detach(), pred_batch_x
