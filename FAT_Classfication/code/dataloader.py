...
def data_generator_only_ft(targetdata_path, configs, training_mode, subset = True):

    train_dataset = torch.load(os.path.join(targetdata_path, "train.pt"))
    val_dataset = torch.load(os.path.join(targetdata_path, "val.pt"))
    ...
    finetune_dataset = Load_Dataset(val_dataset, configs, training_mode, target_dataset_size=configs.target_batch_size, subset=subset)
